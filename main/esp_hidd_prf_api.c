...
void esp_hidd_send_mouse_value(uint16_t conn_id, uint8_t mouse_button, int8_t mickeys_x, int8_t mickeys_y, int8_t wheel, int8_t ac)
{
    uint8_t buffer[HID_MOUSE_IN_RPT_LEN];

    buffer[0] = mouse_button;   // Buttons(left:0x01, right:0x02, middle:0x04)
    buffer[1] = mickeys_x;      // X(-127 ~ +127)
    buffer[2] = mickeys_y;      // Y(-127 ~ +127)
    buffer[3] = wheel;          // Wheel(-127 ~ +127)
    buffer[4] = ac;             // AC Pan(-127 ~ +127)
