...
#define HID_TAG "HID"
#define HIDD_DEVICE_NAME "esp32_KeypadMouse"
...
static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
    switch(event) {
		...
		case ESP_HIDD_EVENT_BLE_CONNECT: {
            ...
            xEventGroupSetBits(device_event, BT_CONNECTED); // connect event
            break;
        }
...
void idleTask(void *pvParameters){
    ...
    while(1){
        ...
        EventBits_t bits = xEventGroupWaitBits(device_event, DEVICE_USED, pdTRUE, pdFALSE, IDLE_TIME / portTICK_PERIOD_MS);
        if((bits & DEVICE_USED) == 0){
            ESP_LOGI(HIDD_DEVICE_NAME,"Enter Deep Sleep\n");
...
void app_main(void){
    ...
    // create device_event
    device_event = xEventGroupCreate();
    ...
    // init button task settings
    init_button_task(device_event);

    // init joystick task settings
    init_joystick(device_event);
    
    // create tasks
    xTaskCreate(buttonTask, "buttonTask", 4096, NULL, 10, NULL);
    xTaskCreate(joystickTask, "joystickTask", 4096, NULL, 10, NULL);
    xTaskCreate(idleTask, "idleTask", 4096, NULL, 10, NULL);
}
